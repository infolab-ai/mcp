...
async def authenticate(ctx: Context = None) -> str:
    """Start Infolab authentication flow and handle callback automatically.

    Returns:
        Success message after authentication
    """
    logger.info("Starting Infolab authentication flow...")
    callback_server = None
    
    try:
        # Start callback server
        callback_server = InfolabCallbackServer(port=3000)
        await callback_server.start()

        # Get auth URL
        logger.debug("Getting authorization URL from Infolab")
        ...
        success_msg = f"Successfully authenticated with Infolab as {user_info.name}!"
...
async def create_post(
    ...
    """
    logger.info("Creating Infolab post...")
    try:
        if ctx:
            ctx.info(f"Creating Infolab post with visibility: {visibility}")
        ...
        # Create the post
        logger.info("Sending post to Infolab API")
        post_id = await post_manager.create_post(post_request)
        success_msg = f"Successfully created Infolab post with ID: {post_id}"
        logger.info(success_msg)
